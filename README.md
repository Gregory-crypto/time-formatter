...
The function named `add_time` that takes in two required parameters and one optional parameter:
...
Assume that the start times are valid times. The minutes in the duration time should be a whole number less than 60, but the hour can be any whole number.
